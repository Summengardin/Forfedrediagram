...
TEST_CASE("Check for future dates")
{
    Date martinBirthday{22, 6, 1999};
    Date mahammedBirthday{15, 5, 1999};
    Date amalieBirthday{21, 8, 1998};
    ...
    Date henningBirthday{29, 7, 2025}; // tesing on after the test date
    Date joachimBirthday{19, 6, 2024};

    CHECK_FALSE(Date::isFutureDate(martinBirthday));
    CHECK_FALSE(Date::isFutureDate(mahammedBirthday));
    CHECK_FALSE(Date::isFutureDate(amalieBirthday));
    CHECK_FALSE(Date::isFutureDate(henrikBirthday));
    CHECK_FALSE(Date::isFutureDate(hassanBirthday));
    CHECK_FALSE(Date::isFutureDate(yallaBirthday));
    CHECK(Date::isFutureDate(henningBirthday));
    CHECK(Date::isFutureDate(joachimBirthday));
}