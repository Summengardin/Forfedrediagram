...
int main() {
    ...
    Menu mainMenu{"HOVEDMENY",
                  {{"Vis tre", [&familyTree]() {
                   ...
                   {"Legg til person", [&familyTree]() {
                        ...
                        // Generate person and Node-object
                       Person newPerson;
                       newPerson.edit();
                       ...
                       while (matchingNodes.empty()) {
                           if(faultCounter >= 2){
                               ...
                               for (auto& node : familyTree.listNodes()){
                       ...
                       if (matchingNodes.size() == 1) {
                           ...
                           Menu matchesMenu;
                           matchesMenu.setTitle("Hvem er " + newNode->getData()->getFirstName() + " forelder til?");
                           for (auto node: familyTree.findNodeByString(name)) {
    ...
    auto inp = COM::getString("A text");
    std::cout << inp << " is " << (Person::validName(inp) ? "valid" : "not valid");
